...
import type { AssetBalance, DashboardData } from '@/lib/types/dashboard';
...
function AssetRows({ assets, emptyMessage, borrowed = false }: { assets: AssetBalance[]; emptyMessage: string; borrowed?: boolean }) {
  if (assets.length === 0) return <p className="text-text-secondary">{emptyMessage}</p>;

  return (
    <Flex direction="col" gap="md">
      {assets.map((asset) => (
        <Flex key={asset.assetSymbol} justify="between" align="center" className="py-2 border-b border-border last:border-0">
          <Flex gap="md" align="center">
...
export async function DepositedAssetsCard({ walletAddress }: { walletAddress: string }) {
  ...
  return (
    <Card>
      ...
      <CardContent><AssetRows assets={result.data.portfolio.depositedAssets} emptyMessage="No deposited assets found." /></CardContent>
    </Card>
  );
}
...
export async function BorrowedAssetsCard({ walletAddress }: { walletAddress: string }) {
  ...
  return (
    <Card>
      ...
      <CardContent><AssetRows assets={result.data.portfolio.borrowedAssets} emptyMessage="No borrowed assets." borrowed /></CardContent>
    </Card>
  );
}
        ...
        {activity.length === 0 ? <p className="text-text-secondary">No recent activity found.</p> : (
          <Flex direction="col" gap="md">
            {activity.slice(0, 20).map((item) => (
              <Flex key={item.id} justify="between" align="center" className="py-3 border-b border-border last:border-0">
                <div>
