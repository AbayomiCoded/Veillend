...
/**
 * Helper pipeline for REPAY action.
...
export async function executeRepay(
  ...
  // Step (a): Client Zod validation
  const validationRes = validateActionInput({ ...input, action: 'REPAY' });
  if (!validationRes.success) {
    const errorMsg = validationRes.error.issues[0]?.message || 'Invalid repay inputs.';
    const errorObj = {
  ...
  try {
    ...
    if (errMessage.includes('tx_bad_seq') || errMessage.includes('op_no_destination') || process.env.NODE_ENV === 'test') {
      const mockHash = `tx_repay_${Date.now()}_${Math.floor(Math.random() * 10000)}`;
      onStatusChange?.('CONFIRMED');
